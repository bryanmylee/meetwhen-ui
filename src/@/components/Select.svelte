<script lang="ts">
  ...
  import scrollToNthChild from "@/utils/popper/scrollToNthChild";
  import type { OptionsGeneric, StrictModifiers } from "@popperjs/core";
  import type { SameWidthModifier } from "@/utils/popper/sameWidth";
  import type { ScrollToNthChildModifier } from "@/utils/popper/scrollToNthChild";
  ...
  const actions = {
    ArrowUp: () => {
      if (showDrop) {
        focusedIndex = decremented(focusedIndex);
      } else {
        index = decremented(index);
      }
    },
    ArrowDown: () => {
      if (showDrop) {
        focusedIndex = incremented(focusedIndex, options.length - 1);
      } else {
        index = incremented(index, options.length - 1);
      }
    },
    Escape: () => {
      showDrop = false;
...
{#if showDrop}
  <div use:popper="{popperOptions}" class="{dropclass}">
