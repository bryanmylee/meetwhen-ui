<script lang="ts">
  ...
  import { hasLeft, hasRight, hasTop, hasBottom } from "@/utils/calendar";
  ...
  $: weekDays = range(1, 8).map((day) => month.day(day));
  $: monthDates = range(1, month.daysInMonth() + 1).map((d) =>
    month.date(d).startOf("day")
  );
  $: monthDateIds = monthDates.map((d) => d.format("YYYYMMDD"));
  $: disabledDateIds = monthDates
    .filter((d) => d.isBefore(dayjs().startOf("day")))
