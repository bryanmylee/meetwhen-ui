...
<script lang="ts">
  ...
  import { primary } from "@/state/colors";
  import { getTop, getHeight } from "@/utils/eventCalendar";
  import type { Dayjs } from "dayjs";
  import type { Scale } from "chroma-js";
  import type { OptionsGeneric, StrictModifiers } from "@popperjs/core";
  import type { IAllUsers } from "@/components/EventCalendar.svelte";
...
{#if focused || hovered}
  <div
    ...
    class="{`fixed text-xs bg-white pointer-events-none card w-max ${
      hovered ? 'z-40' : 'z-30'
    }`}"
  >
