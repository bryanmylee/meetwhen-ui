...
const brightColors = flat(
  hues.map((hue) => sats.map((sat) => chroma.hsl(hue, sat, 0.52).hex()))
);
const greyColors = range(0.3, 0.8, 0.2).map((light) =>
  chroma.hsl(0, 0, light).hex()
);

export const colors = [...brightColors, ...greyColors];
export const DEFAULT_PRIMARY_INDEX = 17;
export const [primaryBase, primary] = useColor(
  "primary",
  colors[DEFAULT_PRIMARY_INDEX]
);
