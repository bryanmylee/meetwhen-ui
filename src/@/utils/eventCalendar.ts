...
import { midnightSplit } from "@/models/Interval";
import type { Dayjs } from "dayjs";
import type Interval from "@/models/Interval";
import type TaggedDay from "@/models/TaggedDay";
import type TimeTaggedDay from "@/models/TimeTaggedDay";
import type UserTaggedInterval from "@/models/UserTaggedInterval";
import type UserTaggedDay from "@/models/UserTaggedDay";
...
export const getUserTaggedDays = (
...
export const extendedTaggedDays = <B extends TaggedDay, E extends TaggedDay>(
  base: B[],
  ...extensions: E[][]
) => {
...
const getActions = (users: Record<string, Interval[]>) => {
  if (users == null) return [];
  const nestedEntries = Object.entries(users).map(([name, schedule]) =>
    schedule.map((interval) => [name, interval] as [string, Interval])
  );
...
export const getUserTaggedIntervals = (
  users: Record<string, Interval[]>
): UserTaggedInterval[] => {
...
export const getTop = (from: Dayjs, hours: Dayjs[]) =>
  getPercentage(from, hours);

export const getHeight = (from: Dayjs, to: Dayjs, hours: Dayjs[]) => {
  return getPercentage(to, hours) - getPercentage(from, hours);
};

export const hasTopInterval = (
  center: UserTaggedInterval,
  schedule: UserTaggedInterval[]
) => {
  const { from, users: centerUsers } = center;
  return schedule.some(
    ({ to, users }) => to.isSame(from) && users.length !== centerUsers.length
  );
};

export const hasBottomInterval = (
  center: UserTaggedInterval,
  schedule: UserTaggedInterval[]
) => {
  const { to, users: centerUsers } = center;
  return schedule.some(
    ({ from, users }) => from.isSame(to) && users.length !== centerUsers.length
  );
};
