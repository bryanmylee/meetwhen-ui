import { fromInterval, toInterval } from "@/models/IntervalTransferObject";
...
export const toEvent = ({
  ...
  users: Object.fromEntries(
    Object.entries(userSchedules).map(([name, schedule]) => [
      name,
      schedule.map(toInterval),
    ])
  ),
});
...
export const fromEvent = ({
  ...
  userSchedules: Object.fromEntries(
    Object.entries(users ?? {}).map(([name, schedule]) => [
      name,
      schedule.map(fromInterval),
    ])
  ),
});
