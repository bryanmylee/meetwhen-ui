...
<Meta title="Input/Select" component={Select} />

<Story name="Sample selects">
	<div class="flex flex-col gap-4">
		...
		<Select
			value="Top position many items"
			values={range(50).map((i) => `${i}`)}
			top
		/>
		...
		<Select
			value="Top position many items small"
			values={range(50).map((i) => `${i}`)}
			top
			sm
		/>
