...
<div class="max-w-lg p-6 mx-auto space-y-4">
  ...
  <div class="p-4 card space-y-4">
    ...
    <form
      on:submit|preventDefault="{() => goto('/new')}"
  ...
  <Accordian classx="{({ expanded }) => (expanded ? 'card' : '')}" let:expanded>
    <div
      slot="title"
      class="{`flex justify-between w-full p-4 ${
        expanded ? '' : 'card interactive'
      }`}"
    >
      <h2>Find an event</h2>
      <ChevronDownIcon
        class="{`p-2 -m-2 icon transition transform ${
          expanded ? 'rotate-180' : ''
        }`}"
      />
    </div>
